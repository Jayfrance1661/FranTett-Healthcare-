...
async function loadVitalSigns(patientId) {
    ...
    if (!container) {
        console.warn(
            "Vital Signs history container not found."
        );
        return;
    }
    ...
    try {
        ...
        const response =
            await fetch(
                `${API_URL}/api/patients/${parsedPatientId}/vitals`,
                {
                    headers: {
                        "Authorization":
                            `Bearer ${token}`
...
async function deleteVitalSigns(id) {
    ...
    if (
        ...
        alert("Invalid vital signs record.");
        return;
    }
